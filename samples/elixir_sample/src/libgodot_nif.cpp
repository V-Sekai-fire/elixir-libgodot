...
static void libgodot_log_callback(LogCallbackData, const char *p_log_message,
                                  bool p_err) {
...
static std::unordered_map<uint64_t, std::shared_ptr<ResponseWaiter>>
    pending_responses;
...
protected:
  static void _bind_methods() {
    ADD_SIGNAL(godot::MethodInfo(
        "new_message", godot::PropertyInfo(godot::Variant::INT, "queued")));
    ...
    godot::ClassDB::bind_method(godot::D_METHOD("send_to_elixir", "msg"),
                                &ElixirBus::send_to_elixir);
    godot::ClassDB::bind_method(
        godot::D_METHOD("send_event", "kind", "payload"),
        &ElixirBus::send_event);
    godot::ClassDB::bind_method(
        godot::D_METHOD("respond", "request_id", "response"),
        &ElixirBus::respond);
  }
...
public:
  ...
  void send_to_elixir(const godot::String &msg) const {
    ...
    ERL_NIF_TERM payload =
        enif_make_string(msg_env, cs.get_data(), ERL_NIF_UTF8);
    ERL_NIF_TERM term = enif_make_tuple2(
        msg_env, enif_make_atom(msg_env, "godot_message"), payload);
  ...
  void send_event(const godot::String &kind,
                  const godot::String &payload) const {
    if (!has_subscriber) {
      return;
    }
    ...
    ERL_NIF_TERM kind_term =
        enif_make_string(msg_env, kind_cs.get_data(), ERL_NIF_UTF8);
    ERL_NIF_TERM payload_term =
        enif_make_string(msg_env, payload_cs.get_data(), ERL_NIF_UTF8);
    ERL_NIF_TERM term =
        enif_make_tuple3(msg_env, enif_make_atom(msg_env, "godot_event"),
                         kind_term, payload_term);
...
initialize_default_module(godot::ModuleInitializationLevel p_level) {
  ...
  if (!elixir_bus_singleton) {
    ...
    godot::Engine::get_singleton()->register_singleton(
        godot::StringName("ElixirBus"), elixir_bus_singleton);
  }
}
...
uninitialize_default_module(godot::ModuleInitializationLevel p_level) {
  ...
  if (elixir_bus_singleton) {
    godot::Engine::get_singleton()->unregister_singleton(
        godot::StringName("ElixirBus"));
...
GDExtensionObjectPtr (*worker_create_instance)(
    int, char *[], GDExtensionInitializationFunction, InvokeCallbackFunction,
    ExecutorData, InvokeCallbackFunction, ExecutorData, LogCallbackFunction,
...
static void set_result(const std::shared_ptr<Request> &req, bool ok,
                       const std::string &err = "", bool quit = false) {
...
static bool args_to_argv(const std::vector<std::string> &args,
...
#ifdef __APPLE__
static void
maybe_enable_embedded_headless(const std::vector<std::string> &args) {
  for (const auto &s : args) {
    if (s == "--headless") {
...
static void worker_loop() {
  for (;;) {
    std::shared_ptr<Request> req;
    {
      ...
      queue_cv.wait(lk, [] {
        return !queue.empty() ||
               incoming_messages_pending.load(std::memory_order_acquire);
      });

      // If a message arrived, emit the signal immediately on this (Godot)
      // thread. Do not force a full frame iteration for message delivery.
      if (incoming_messages_pending.exchange(false,
                                             std::memory_order_acq_rel) &&
          elixir_bus_singleton) {
        int32_t queued = 0;
        {
          std::lock_guard<std::mutex> lk2(incoming_messages_mutex);
          queued = static_cast<int32_t>(incoming_messages.size());
        }
        elixir_bus_singleton->emit_signal(godot::StringName("new_message"),
                                          queued);
      }
    ...
    case RequestType::CreateWithPath: {
      ...
      *(void **)(&worker_create_instance) =
          dlsym(worker_handle, "libgodot_create_godot_instance");
      *(void **)(&worker_destroy_instance) =
          dlsym(worker_handle, "libgodot_destroy_godot_instance");
      ...
      GDExtensionObjectPtr obj = worker_create_instance(
          static_cast<int>(argv_c.size() - 1), argv_c.data(),
          gdextension_default_init, nullptr, nullptr, nullptr, nullptr, log_fn,
          log_data);
      if (!obj) {
        dlclose(worker_handle);
        worker_handle = nullptr;
      ...
      worker_object = obj;
      worker_instance = reinterpret_cast<godot::GodotInstance *>(
...
static void ensure_worker_started() {
  std::call_once(worker_once, [] { std::thread(worker_loop).detach(); });
}

static std::shared_ptr<Request>
call_worker(RequestType type, const std::vector<std::string> &args,
...
static ERL_NIF_TERM make_error(ErlNifEnv *env, const std::string &msg) {
  return enif_make_tuple2(env, enif_make_atom(env, "error"),
                          enif_make_string(env, msg.c_str(), ERL_NIF_UTF8));
}
...
static ERL_NIF_TERM request(ErlNifEnv *env, int argc,
  ...
  {
    std::lock_guard<std::mutex> lk(incoming_messages_mutex);
    incoming_messages.emplace_back("__request__:" + std::to_string(id) + ":" +
                                   payload);
  }
  ...
  bool ok = waiter->cv.wait_for(lk, std::chrono::milliseconds(timeout_ms),
                                [&] { return waiter->done; });
  if (!ok) {
  ...
  ERL_NIF_TERM resp_term =
      enif_make_string(env, response.c_str(), ERL_NIF_UTF8);
  return enif_make_tuple2(env, enif_make_atom(env, "ok"), resp_term);
}

// send_message(ref, binary|string) -> :ok | {:error, reason}
static ERL_NIF_TERM send_message(ErlNifEnv *env, int argc,
                                 const ERL_NIF_TERM argv[]) {
  (void)argc;
  GodotNifResource *res = nullptr;
  if (!enif_get_resource(env, argv[0], GODOT_RES_TYPE, (void **)&res) || !res) {
...
static bool term_to_string(ErlNifEnv *env, ERL_NIF_TERM term,
                           std::string &out) {
  ...
  if (enif_is_list(env, term)) {
    ...
    if (enif_get_list_length(env, term, &len)) {
      ...
      if (enif_get_string(env, term, buf.data(),
...
static bool list_to_argv(ErlNifEnv *env, ERL_NIF_TERM list_term,
...
static ERL_NIF_TERM subscribe(ErlNifEnv *env, int argc,
                              const ERL_NIF_TERM argv[]) {
  (void)argc;
  ErlNifPid pid;
...
static ERL_NIF_TERM create(ErlNifEnv *env, int argc,
  ...
  auto *res = static_cast<GodotNifResource *>(
      enif_alloc_resource(GODOT_RES_TYPE, sizeof(GodotNifResource)));
  if (!res) {
...
static ERL_NIF_TERM create_with_path(ErlNifEnv *env, int argc,
                                     const ERL_NIF_TERM argv[]) {
  (void)argc;
  ...
  auto result =
      call_worker(RequestType::CreateWithPath, arg_storage, libgodot_path);
  if (!result->ok) {
    return make_error(env, result->err);
  }
...
static ERL_NIF_TERM start(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]) {
  ...
  if (has_subscriber) {
    ...
    ERL_NIF_TERM msg =
        enif_make_tuple2(env, enif_make_atom(env, "godot_status"),
...
static ERL_NIF_TERM iteration(ErlNifEnv *env, int argc,
                              const ERL_NIF_TERM argv[]) {
  (void)argc;
  GodotNifResource *res = nullptr;
  if (!enif_get_resource(env, argv[0], GODOT_RES_TYPE, (void **)&res) || !res) {
...
// shutdown(ref) -> :ok
static ERL_NIF_TERM shutdown(ErlNifEnv *env, int argc,
                             const ERL_NIF_TERM argv[]) {
  (void)argc;
  GodotNifResource *res = nullptr;
  if (!enif_get_resource(env, argv[0], GODOT_RES_TYPE, (void **)&res) || !res) {
...
static int nif_load(ErlNifEnv *env, void **priv_data, ERL_NIF_TERM load_info) {
  ...
  (void)load_info;
  GODOT_RES_TYPE =
      enif_open_resource_type(env, nullptr, "libgodot_resource",
                              godot_resource_dtor, ERL_NIF_RT_CREATE, nullptr);
  ensure_worker_started();
