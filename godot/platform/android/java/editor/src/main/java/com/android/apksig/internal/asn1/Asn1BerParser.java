...
public final class Asn1BerParser {
			...
			case SEQUENCE: {
				...
				if ((container.getTagClass() != expectedTagClass) || (container.getTagNumber() != expectedTagNumber)) {
					throw new Asn1UnexpectedTagException(
							"Unexpected data value read as " + containerClass.getName() + ". Expected " + BerEncoding.tagClassAndNumberToString(expectedTagClass, expectedTagNumber) + ", but read: " + BerEncoding.tagClassAndNumberToString(container.getTagClass(), container.getTagNumber()));
				}
	...
	private static <T> T parseChoice(BerDataValue dataValue, Class<T> containerClass)
		...
		if (fields.isEmpty()) {
			throw new Asn1DecodingException(
					"No fields annotated with " + Asn1Field.class.getName() + " in CHOICE class " + containerClass.getName());
		}
			...
			for (int j = i + 1; j < fields.size(); j++) {
				...
				if ((tagNumber1 == tagNumber2) && (tagClass1 == tagClass2)) {
					throw new Asn1DecodingException(
							...
							+ " class and number: " + containerClass.getName() + "." + f1.getField().getName() + " and ." + f2.getField().getName());
				}
			}
		}
		...
		if (fields.size() > 1) {
			...
			for (AnnotatedField field : fields) {
				if ((lastField != null) && (lastField.getAnnotation().index() == field.getAnnotation().index())) {
					throw new Asn1DecodingException(
							"Fields have the same index: " + containerClass.getName() + "." + lastField.getField().getName() + " and ." + field.getField().getName());
				}
				lastField = field;
			}
		}
		...
		while (nextUnreadFieldIndex < fields.size()) {
			...
			for (int i = nextUnreadFieldIndex; i < fields.size(); i++) {
				...
				try {
					...
					throw new Asn1DecodingException(
							"Failed to parse " + containerClass.getName() + "." + field.getField().getName(),
							e);
				}
			}
		}
	...
	private static Asn1Type getContainerAsn1Type(Class<?> containerClass)
		...
		if (containerAnnotation == null) {
			throw new Asn1DecodingException(
					containerClass.getName() + " is not annotated with " + Asn1Class.class.getName());
		}
		...
		switch (containerAnnotation.type()) {
			...
			default:
				throw new Asn1DecodingException(
						"Unsupported ASN.1 container annotation type: " + containerAnnotation.type());
		}
	}
		...
		public void setValueFrom(BerDataValue dataValue, Object obj) throws Asn1DecodingException {
			...
			if (mBerTagNumber != -1) {
				...
				if ((readTagClass != mBerTagClass) || (readTagNumber != mBerTagNumber)) {
					throw new Asn1UnexpectedTagException(
							"Tag mismatch. Expected: " + BerEncoding.tagClassAndNumberToString(mBerTagClass, mBerTagNumber) + ", but found " + BerEncoding.tagClassAndNumberToString(readTagClass, readTagNumber));
				}
			} else {
				if (readTagClass != mBerTagClass) {
					throw new Asn1UnexpectedTagException(
							"Tag mismatch. Expected class: " + BerEncoding.tagClassToString(mBerTagClass) + ", but found " + BerEncoding.tagClassToString(readTagClass));
				}
			}
	...
	private static List<AnnotatedField> getAnnotatedFields(Class<?> containerClass)
		...
		for (Field field : declaredFields) {
			...
			if (Modifier.isStatic(field.getModifiers())) {
				throw new Asn1DecodingException(
						Asn1Field.class.getName() + " used on a static field: " + containerClass.getName() + "." + field.getName());
			}
			...
			try {
				...
				throw new Asn1DecodingException(
						"Invalid ASN.1 annotation on " + containerClass.getName() + "." + field.getName(),
						e);
			}
	...
	private static final class BerToJavaConverter {
		...
		public static void setFieldValue(
			...
			try {
				...
				throw new Asn1DecodingException(
						"Failed to set value of " + obj.getClass().getName() + "." + field.getName(),
						e);
			}
		}
				...
				case BOOLEAN:
					...
					if (boolean.class.equals(targetType)) {
						if (encodedContents.remaining() != 1) {
							throw new Asn1DecodingException(
									"Incorrect encoded size of boolean value: " + encodedContents.remaining());
						}
						boolean result;
