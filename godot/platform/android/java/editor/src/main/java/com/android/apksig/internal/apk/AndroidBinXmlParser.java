			...
			switch (chunk.getType()) {
				...
				case Chunk.RES_XML_TYPE_START_ELEMENT: {
					...
					if (contents.remaining() < 20) {
						throw new XmlParserException(
								"Start element chunk too short. Need at least 20 bytes. Available: " + contents.remaining() + " bytes");
					}
					...
					if (attrStartOffset > contents.remaining()) {
						throw new XmlParserException(
								"Attributes start offset out of bounds: " + attrStartOffset + ", max: " + contents.remaining());
					}
					if (attrEndOffset > contents.remaining()) {
						throw new XmlParserException(
								"Attributes end offset out of bounds: " + attrEndOffset + ", max: " + contents.remaining());
					}
				...
				case Chunk.RES_XML_TYPE_END_ELEMENT: {
					...
					if (contents.remaining() < 8) {
						throw new XmlParserException(
								"End element chunk too short. Need at least 8 bytes. Available: " + contents.remaining() + " bytes");
					}
					long nsId = getUnsignedInt32(contents);
					long nameId = getUnsignedInt32(contents);
					mCurrentElementName = mStringPool.getString(nameId);
	...
	private static class Chunk {
		...
		public static Chunk get(ByteBuffer input) throws XmlParserException {
			...
			if (headerSize < HEADER_MIN_SIZE_BYTES) {
				...
				throw new XmlParserException(
						"Malformed chunk: header too long: " + headerSize + " bytes. Chunk size: " + chunkSize + " bytes");
			}
	...
	private static class StringPool {
		...
		public StringPool(Chunk chunk) throws XmlParserException {
			...
			if (header.remaining() < 20) {
				throw new XmlParserException(
						"XML chunk's header too short. Required at least 20 bytes. Available: " + header.remaining() + " bytes");
			}
			...
			if (mStringCount > 0) {
				...
				if (styleCount > 0) {
					...
					if (stylesStartOffset < stringsStartOffset) {
						throw new XmlParserException(
								"Styles offset (" + stylesStartOffset + ") < strings offset (" + stringsStartOffset + ")");
					}
		...
		public String getString(long index) throws XmlParserException {
			...
			if (offsetInStringsSection >= mStringsSection.capacity()) {
				throw new XmlParserException(
						"Offset of string idx " + idx + " out of bounds: " + offsetInStringsSection + ", max: " + (mStringsSection.capacity() - 1));
			}
