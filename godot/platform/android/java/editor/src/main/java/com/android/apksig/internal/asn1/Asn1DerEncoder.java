	...
	private static byte[] toChoice(Object container) throws Asn1EncodingException {
		...
		for (AnnotatedField field : fields) {
			...
			if (fieldValue != null) {
				if (resultField != null) {
					throw new Asn1EncodingException(
							"Multiple non-null fields in CHOICE class " + containerClass.getName() + ": " + resultField.getField().getName() + ", " + field.getField().getName());
				}
				resultField = field;
			}
		}
	...
	private static byte[] toSequence(Object container, boolean omitTag)
		...
		for (AnnotatedField field : fields) {
			...
			try {
				...
				throw new Asn1EncodingException(
						"Failed to encode " + containerClass.getName() + "." + field.getField().getName(),
						e);
			}
