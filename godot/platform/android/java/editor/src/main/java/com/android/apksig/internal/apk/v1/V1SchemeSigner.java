	...
	public static List<Pair<String, byte[]>> sign(
		...
		if (signerConfigs.size() > MAX_APK_SIGNERS) {
			throw new IllegalArgumentException(
					"APK Signature Scheme v1 only supports a maximum of " + MAX_APK_SIGNERS + ", " + signerConfigs.size() + " provided");
		}
	...
	public static List<Pair<String, byte[]>> signManifest(
		...
		for (SignerConfig signerConfig : signerConfigs) {
			...
			String signatureBlockFileName =
					"META-INF/" + signerName + "." + publicKey.getAlgorithm().toUpperCase(Locale.US);
