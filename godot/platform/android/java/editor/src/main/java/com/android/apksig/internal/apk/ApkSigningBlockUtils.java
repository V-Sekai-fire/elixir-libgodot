...
public class ApkSigningBlockUtils {
	...
	private static final ContentDigestAlgorithm[] V4_CONTENT_DIGEST_ALGORITHMS = { CHUNKED_SHA512, VERITY_CHUNKED_SHA256, CHUNKED_SHA256 };
	...
	public static void verifyIntegrity(
		...
		if (!contentDigestAlgorithms.equals(actualContentDigests.keySet())) {
			throw new RuntimeException(
					...
					+ " . Requested: " + contentDigestAlgorithms + ", computed: " + actualContentDigests.keySet());
		}
			...
			while (inputRemaining > 0) {
				...
				for (int i = 0; i < digestAlgorithmsArray.length; i++) {
					...
					if (actualDigestSizeBytes != expectedDigestSizeBytes) {
						throw new RuntimeException(
								"Unexpected output size of " + md.getAlgorithm() + " digest: " + actualDigestSizeBytes);
					}
				}
		...
		if (encodedPublicKey == null) {
			try {
				...
				throw new InvalidKeyException(
						"Failed to obtain X.509 encoded form of public key " + publicKey + " of class " + publicKey.getClass().getName(),
						e);
			}
		}
	...
	public static List<Pair<byte[], Integer>> getApkSignatureBlocks(
		...
		while (apkSigningBlockBuffer.hasRemaining()) {
			...
			if (blockLength > Integer.MAX_VALUE || blockLength < 4) {
				throw new IllegalArgumentException(
						"Block index " + (signatureBlocks.size() + 1) + " size out of range: " + blockLength);
			}
		...
		while (signersBuffer.hasRemaining()) {
			...
			while (certificatesBuffer.hasRemaining()) {
				...
				if (certLength > certificatesBuffer.remaining()) {
					throw new IllegalArgumentException(
							"Cert index " + (certificates.size() + 1) + " under signer index " + (signers.size() + 1) + " size out of range: " + certLength);
				}
