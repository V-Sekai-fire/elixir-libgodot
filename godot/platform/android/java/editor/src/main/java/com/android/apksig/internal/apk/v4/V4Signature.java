...
public class V4Signature {
	...
	public static class HashingInfo {
		...
		byte[] toByteArray() {
			final int size = 4 /*hashAlgorithm*/ + 1 /*log2BlockSize*/ + bytesSize(this.salt) + bytesSize(this.rawRootHash);
	...
	public static class SigningInfo {
		...
		byte[] toByteArray() {
			final int size = bytesSize(this.apkDigest) + bytesSize(this.certificate) + bytesSize(this.additionalData) + bytesSize(this.publicKey) + 4 /*signatureAlgorithmId*/
	...
	static byte[] getSignedData(long fileSize, HashingInfo hashingInfo, SigningInfo signingInfo) {
		final int size =
				4 /*size*/ + 8 /*fileSize*/ + 4 /*hash_algorithm*/ + 1 /*log2_blocksize*/ + bytesSize(hashingInfo.salt) + bytesSize(hashingInfo.rawRootHash) + bytesSize(signingInfo.apkDigest) + bytesSize(signingInfo.certificate) + bytesSize(signingInfo.additionalData);
		ByteBuffer buffer = ByteBuffer.allocate(size).order(ByteOrder.LITTLE_ENDIAN);
		buffer.putInt(size);
