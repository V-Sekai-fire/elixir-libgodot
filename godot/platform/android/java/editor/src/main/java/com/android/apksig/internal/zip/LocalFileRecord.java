	...
	private static LocalFileRecord getRecord(
		...
		if (headerEndOffset > cdStartOffset) {
			throw new ZipFormatException(
					...
					+ " Directory. LFH end: " + headerEndOffset + ", CD start: " + cdStartOffset);
		}
		ByteBuffer header;
		try {
		...
		if (recordSignature != RECORD_SIGNATURE) {
			throw new ZipFormatException(
					"Not a Local File Header record for entry " + entryName + ". Signature: 0x" + Long.toHexString(recordSignature & 0xffffffffL));
		}
		...
		if (dataDescriptorUsed != cdDataDescriptorUsed) {
			throw new ZipFormatException(
					...
					+ " Directory for entry " + entryName + ". LFH: " + dataDescriptorUsed + ", CD: " + cdDataDescriptorUsed);
		}
		...
		if (!dataDescriptorUsed) {
			...
			if (crc32 != uncompressedDataCrc32FromCdRecord) {
				throw new ZipFormatException(
						"CRC-32 mismatch between Local File Header and Central Directory for entry " + entryName + ". LFH: " + crc32 + ", CD: " + uncompressedDataCrc32FromCdRecord);
			}
			...
			if (compressedSize != compressedDataSizeFromCdRecord) {
				throw new ZipFormatException(
						...
						+ " for entry " + entryName + ". LFH: " + compressedSize + ", CD: " + compressedDataSizeFromCdRecord);
			}
			...
			if (uncompressedSize != uncompressedDataSizeFromCdRecord) {
				throw new ZipFormatException(
						...
						+ " for entry " + entryName + ". LFH: " + uncompressedSize + ", CD: " + uncompressedDataSizeFromCdRecord);
			}
		}
		int nameLength = ZipUtils.getUnsignedInt16(header, NAME_LENGTH_OFFSET);
		if (nameLength > cdRecordEntryNameSizeBytes) {
			throw new ZipFormatException(
					"Name mismatch between Local File Header and Central Directory for entry" + entryName + ". LFH: " + nameLength + " bytes, CD: " + cdRecordEntryNameSizeBytes + " bytes");
		}
		String name = CentralDirectoryRecord.getName(header, NAME_OFFSET, nameLength);
		if (!entryName.equals(name)) {
			throw new ZipFormatException(
					"Name mismatch between Local File Header and Central Directory. LFH: \"" + name + "\", CD: \"" + entryName + "\"");
		}
		...
		if (dataEndOffset > cdStartOffset) {
			throw new ZipFormatException(
					...
					+ ". LFH data start: " + dataStartOffset + ", LFH data end: " + dataEndOffset + ", CD start: " + cdStartOffset);
		}
		...
		if ((dataDescriptorIncluded) && ((gpFlags & ZipUtils.GP_FLAG_DATA_DESCRIPTOR_USED) != 0)) {
			...
			if (dataDescriptorEndOffset > cdStartOffset) {
				throw new ZipFormatException(
						...
						+ ". Data Descriptor end: " + dataEndOffset + ", CD start: " + cdStartOffset);
			}
	...
	public void outputUncompressedData(
		...
		try {
			if (mDataCompressed) {
				try (InflateSinkAdapter inflateAdapter = new InflateSinkAdapter(sink)) {
					...
					if (actualUncompressedSize != mUncompressedDataSize) {
						throw new ZipFormatException(
								"Unexpected size of uncompressed data of " + mName + ". Expected: " + mUncompressedDataSize + " bytes"
			...
			throw new IOException(
					"Failed to read data of " + ((mDataCompressed) ? "compressed" : "uncompressed") + " entry " + mName,
					e);
		}
