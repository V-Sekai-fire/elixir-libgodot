...
public class ApkSigner {
	...
	private void sign(DataSource inputApk, DataSink outputApkOut, DataSource outputApkIn)
		...
		if (outputCentralDirSizeBytes > Integer.MAX_VALUE) {
			throw new IOException(
					"Output ZIP Central Directory too large: " + outputCentralDirSizeBytes + " bytes");
		}
		...
		if (outputApkSigningBlockRequest != null) {
			...
			long fileSize =
					outputCentralDirStartOffset + outputCentralDirDataSource.size() + padding + outputApkSigningBlock.length + outputEocd.remaining();
			if (mAlignFileSize && (fileSize % ANDROID_FILE_ALIGNMENT_BYTES != 0)) {
				int eocdPadding =
						(int)(ANDROID_FILE_ALIGNMENT_BYTES - fileSize % ANDROID_FILE_ALIGNMENT_BYTES);
	...
	private OutputSizeAndDataOffset outputInputJarEntryLfhRecord(
		...
		if ((dataAlignmentMultiple <= 1) || ((inputOffset % dataAlignmentMultiple) == (outputOffset % dataAlignmentMultiple) && mAlignmentPreserved)) {
			// This record's data will be aligned same as in the input APK.
			return new OutputSizeAndDataOffset(
					inputRecord.outputRecord(inputLfhSection, outputLfhSection),
					inputRecord.getDataStartOffsetInRecord());
		...
		long dataOffset =
				(long)inputRecord.getDataStartOffsetInRecord() + aligningExtra.remaining() - inputRecord.getExtra().remaining();
		return new OutputSizeAndDataOffset(
	...
	private static ByteBuffer createExtraFieldToAlignData(
		...
		while (original.remaining() >= 4) {
			...
			if (((headerId == 0) && (dataSize == 0)) || (headerId == ALIGNMENT_ZIP_EXTRA_DATA_FIELD_HEADER_ID)) {
		...
		long dataMinStartOffset =
				extraStartOffset + result.position() + ALIGNMENT_ZIP_EXTRA_DATA_FIELD_MIN_SIZE_BYTES;
		int paddingSizeBytes =
				(dataAlignmentMultiple - ((int)(dataMinStartOffset % dataAlignmentMultiple))) % dataAlignmentMultiple;
	...
	private static List<CentralDirectoryRecord> parseZipCentralDirectory(
		...
		for (int i = 0; i < expectedCdRecordCount; i++) {
			...
			try {
				...
				throw new ApkFormatException(
						"Malformed ZIP Central Directory record #" + (i + 1) + " at file offset " + (cdOffset + offsetInsideCd),
						e);
			}
		...
		if (cd.hasRemaining()) {
			throw new ApkFormatException(
					"Unused space at the end of ZIP Central Directory: " + cd.remaining() + " bytes starting at file offset " + (cdOffset + cd.position()));
		}
