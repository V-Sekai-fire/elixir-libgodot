...
public class V3SchemeVerifier {
	...
	public ApkSigningBlockUtils.Result verify()
		...
		for (ApkSigningBlockUtils.Result.SignerInfo signer : sortedSigners.values()) {
			...
			if (firstMin == 0) {
				...
				if (currentMin != (lastMax + 1) && !(currentMin == lastMax && signerTargetsDevRelease(signer))) {
		...
		if (firstMin > mMinSdkVersion || lastMax < (mOptionalRotationMinSdkVersion.isPresent() ? mOptionalRotationMinSdkVersion.getAsInt() - 1 : mMaxSdkVersion)) {
	...
	public static void parseSigners(
		...
		try {
			...
			throw new IllegalStateException("An exception was encountered when attempting to parse"
							+ " the signers from the provided APK Signature Scheme v3 block",
					e);
		}
	}
	...
	private void parseSigner(ByteBuffer signerBlock, CertificateFactory certFactory,
		...
		for (ApkSigningBlockUtils.SupportedSignature signature : signaturesToVerify) {
			...
			try {
				publicKey =
						KeyFactory.getInstance(keyAlgorithm).generatePublic(new X509EncodedKeySpec(publicKeyBytes));
			} catch (Exception e) {
		...
		while (additionalAttributes.hasRemaining()) {
			...
			try {
				...
				if (id == V3SchemeConstants.PROOF_OF_ROTATION_ATTR_ID) {
					...
					if (mMaxSdkVersion >= V3SchemeConstants.MIN_SDK_WITH_V31_SUPPORT && mFullVerification) {
